...
export function extractRangoliCenter(design, start, end) {
  ...
  if (start < 0 || end < 0 || start >= design.length || end > design.length || start >= end) {
    return "";
  }
...
export function splitAndJoinRangoli(colorString, oldSep, newSep) {
 ...
 let  newSarr = colorString.split(oldSep)

  let newString=  newSarr.join(newSep)
...
export function makeRangoliBorder(char, length) {
   ...
   let repeatChar = char.repeat(Math.ceil(length / char.length)).slice(0, length);
